...
const crmCustomerSchema = new mongoose.Schema({
  ...
  contacts: [{
    ...
    email: String,
    phone: String,
    role: { type: String, enum: ["beslut", "anvandare", "ekonomi", "kontakt"] }
  }],
  mrr: { type: Number, default: 0 },
  arr: { type: Number, default: 0 },
  grossMargin: { type: Number, default: 0 },
  totalValue: { type: Number, default: 0 },
  aiCostTotal: { type: Number, default: 0 },
  ...
  aiConfig: {
    ...
    policy: { type: String, default: "standard" }
  },
  address: {
...
const crmDealSchema = new mongoose.Schema({
  ...
  value: { type: Number, default: 0 },
  stage: { type: String, default: "new" }, // new, qualified, proposal, negotiation, won, lost
  ...
  // ADVANCED FIELDS
  weightedValue: { type: Number, default: 0 }, // value * probability
  lossReason: { type: String, default: "" }, // pris, konkurrent, behov, etc
  lossCompetitor: { type: String, default: "" },
  lossComment: { type: String, default: "" },
...
const crmActivitySchema = new mongoose.Schema({
  companyId: { type: String, required: true, index: true },
  id: { type: String, required: true },
  type: { type: String, default: "call" }, // call, meeting, email, task
  subject: { type: String, required: true },
  description: { type: String, default: "" },
  date: { type: String, default: "" },
  status: { type: String, default: "planned" }, // planned, done
  targetId: { type: String, default: "" }, // Customer or Deal ID
  assignedTo: { type: String, default: "me" },
...
app.post("/crm/customers/sync", authenticate, async (req, res) => {
  try {
    ...
    io.emit("crm_update", { type: "customers" });
...
app.post("/crm/activities/sync", authenticate, async (req, res) => {
  try {
    ...
    io.emit("crm_update", { type: "activities" });
